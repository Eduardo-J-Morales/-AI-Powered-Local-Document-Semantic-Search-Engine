	...
	async function postFileMetadata(metadata) {
		try {
			const response = await fetch(
				"http://localhost:5191/api/documents/upload",
				{
					method: "POST",
					headers: {
		...
		if (files && files.length > 0) {
			...
			fileInfo = {
				filename:
					amount == 0
						? selectedFile.name
						: selectedFile.name + ` (${amount})`,
				contentType: selectedFile.type,
				size: selectedFile.size,
				route:
					(selectedFile as any).webkitRelativePath ||
					"(not available)",
				tags: [],
			};
	...
	async function deleteFile(file) {
		try {
			const res = await fetch(
				`http://localhost:5191/api/documents/${file}`,
				{ method: "DELETE" },
			);
			if (!res.ok) {
				const error = await response.json();
					...
					<div class="file-card">
						<div class="file-card-header">
							...
							<div class="file-card-actions">
								<button
									class="icon-btn"
									on:click={() => deleteFile(info.filename)}
									>❌</button
								>
								<button
									class="icon-btn"
									on:click={() => openTagModal(idx)}
									>🏷️</button
								>
							</div>
						</div>
